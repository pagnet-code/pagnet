...
// writeContract (re)renders the AgentNet coordination contract (§16:
...
func (d *Daemon) writeContract(row *InstanceRow) (string, error) {
	...
	text := fmt.Sprintf(`AGENTNET COORDINATION CONTRACT

You are a persistent member of an AgentNet network.
...
2. AGENTNET NETWORK PEERS
   These are persistent agents registered in AgentNet and may run on
...
AgentNet peer instead of modifying that resource yourself.

Your AgentNet identity:
  agent:    %s
  network:  %s
  instance: %s

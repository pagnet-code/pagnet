...
func TestLoadDaemon_DoesNotSetenvEnvFileValues(t *testing.T) {
	...
	if err := os.WriteFile(".env", []byte(
		"AGENTNET_SERVER=http://127.0.0.1:19999\n"+
			"DATABASE_URL=postgres://must-not-leak\n"+
			"AGENTNET_ADMIN_TOKEN=must-not-leak\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	// Force a clean slate for the keys the test asserts on.
	t.Setenv("AGENTNET_SERVER", "")
	t.Setenv("DATABASE_URL", "")
	t.Setenv("AGENTNET_ADMIN_TOKEN", "")
	...
	if os.Getenv("AGENTNET_ADMIN_TOKEN") != "" {
		t.Fatal("AGENTNET_ADMIN_TOKEN must NOT be set in the daemon process env")
	}
	if os.Getenv("AGENTNET_SERVER") != "" {
		t.Fatal("AGENTNET_SERVER must NOT be set in the daemon process env")
	}
}
...
func TestLoadDaemon_ProcessEnvWinsOverFile(t *testing.T) {
	dir := t.TempDir()
	t.Chdir(dir)
	if err := os.WriteFile(".env", []byte("AGENTNET_SERVER=http://from-file:1\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	t.Setenv("AGENTNET_SERVER", "http://from-env:2")
	cfg, err := LoadDaemon(filepath.Join(dir, "state"))
	if err != nil {
		t.Fatalf("LoadDaemon: %v", err)
	}
